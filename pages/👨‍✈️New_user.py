...
pickleFile=pickle.load(open("weights.pkl","rb"))
...
def main():
  ...
  html2="""
    <div style="text-align:center; text-shadow: 3px 1px 2px purple;">
      <h2>?? Know Your Career ??<h2>
    </div>
      """
  # st.markdown("Predction.py", label="Home", icon="🏠")
  ...
  Name = st.sidebar.text_input("University Name ")
  ...
  if st.button("Predict"):
    ...
    corr = df[['Logical quotient rating', 'hackathons', 'coding skills rating', 'public speaking points']].corr()

# Calculate absolute correlation values for each feature pair
    abs_corr = corr.abs().sum(axis=1)

# Plot pie chart
    fig, ax = plt.subplots(figsize=(8, 8))
    abs_corr.plot(kind='pie', ax=ax, autopct='%1.1f%%')

# Add title
    plt.title('Sum of Absolute Correlation')

# Add legend
    plt.legend(abs_corr.index, loc='upper right')
...
# Equal aspect ratio ensures that pie is drawn as a circle
    ...
    # plt.legend(title='Feature Pairs', loc='upper right')
    # st.pyplot(plt)
  ...
  html3="""
    ...
    <div style="color:yellow; margin:80px; text-align:center;">
      Developed with ❤️ by <a href=https://github.com/Tushar-CYL>Tushar & Team</a>
    </div>
      """
